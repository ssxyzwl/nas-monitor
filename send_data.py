...
BAUD_RATE = 9600
RECONNECT_INTERVAL = 2  # 重连检测间隔（秒）
UART_SETTLE_DELAY = 0.1  # 重连后丢弃残余数据前的等待（秒）
...
_cached_interface = None  # 缓存自动检测到的网卡名
...
def _get_default_route_interface():
    """从默认路由获取出口网卡名"""
    # 方法1：读 /proc/net/route，不需要外部命令
    try:
        with open('/proc/net/route', 'r') as f:
            for line in f.readlines():
                fields = line.strip().split()
                if len(fields) >= 2 and fields[1] == '00000000':
                    # 目标 00000000 = 默认路由，fields[0] = 网卡名
                    iface = fields[0]
...
def _get_busiest_physical_interface():
    """从 /proc/net/dev 中找流量最大的物理网卡"""
    # 虚拟/容器网卡前缀，应排除
    VIRTUAL_PREFIXES = ('lo', 'docker', 'br-', 'veth', 'virbr', 'vmnet',
                        'flannel', 'cni', 'cali', 'tun', 'tap', 'wg')
    try:
...
def _match_fallback_interface():
    ...
    FALLBACK_PATTERNS = [
        'eth0', 'eth1',
        'ens33', 'ens160', 'ens192', 'ens256',
        'eno1', 'eno2',
        'enp3s0', 'enp0s3', 'enp0s25', 'enp2s0',
        'en0',  # macOS
    ]
    try:
        with open('/proc/net/dev', 'r') as f:
        ...
        for entry in os.listdir('/proc'):
            if entry.isdigit():
                ...
                try:
                    ...
                    pass
        if proc_count == 0:
            result = subprocess.run(['ps', '-e', '--no-headers'],capture_output=True, text=True, timeout=2)
            if result.returncode == 0:
                lines = result.stdout.strip().split('\n')
                proc_count = len(lines) if lines[0] else 0
            result = subprocess.run(['ps', '-e', '--no-headers', '--state=R'],capture_output=True, text=True, timeout=2)
            if result.returncode == 0:
                lines = result.stdout.strip().split('\n')
                run_count = len(lines) if lines[0] else 0
                    ...
                    if len(parts) >= 10:
                        for part in reversed(parts):
                            if part.isdigit():
                                return int(part)
        temp_paths = [
            f'/sys/class/block/{os.path.basename(disk_device)}/device/hwmon/hwmon*/temp1_input',
            f'/sys/class/block/{os.path.basename(disk_device)}/device/temp1_input',
...
def get_memory_usage():
    ...
    if mem_info['total'] > 0:
        ...
        usage_percent = (used / mem_info['total']) * 100
...
def get_network_speed(interface=None):
    ...
    rx_speed_kb = round(rx_speed / 1024, 2)
    tx_speed_kb = round(tx_speed / 1024, 2)
    return tx_speed_kb, rx_speed_kb
...
try:
    ...
    while True:
        ...
        if current_total > last_total:
            ...
            cpu_percent = (cpu_used / total_delta) * 100 if total_delta > 0 else 0
        ...
        system_data = {
            ...
            "tx": tx_speed,
            "rx": rx_speed,
            "smb": 1 if smb_status else 0,
            "nfs": 1 if nfs_status else 0,
            "ssh": 1 if ssh_status else 0,
            "temp": disk_temp,
            "health": disk_health,
            "part_mount": current_partition['mount'],
            "part_total": current_partition['total'],
            "part_used": current_partition['used'],
            "part_available": current_partition['available'],
            "part_percent": current_partition['percent'],
            "proc_total": proc_total,
            "proc_run": proc_running,
            "cpu_model": cpu_model,
            "mem_type": mem_type
